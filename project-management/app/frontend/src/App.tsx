  ...
  const handleResumeSession = useCallback(async (ps: PausedSession) => {
    try {
      ...
      setSession({
        ...
        config: { session_type: 'mixed', domains: ps.domains, question_count: ps.questions_total, difficulty: 'mixed', timed: true, time_per_question: 90 },
        results: [],
      });
      ...
      <header className="sticky top-0 z-50 border-b border-white/5" style={{ background: 'rgba(15, 23, 42, 0.85)', backdropFilter: 'blur(20px)' }}>
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-teal-500 to-emerald-600 flex items-center justify-center text-white font-bold text-sm shadow-lg shadow-teal-500/20">P</div>
            <h1 className="text-lg font-semibold text-white tracking-tight">PM Practice</h1>
          ...
          <nav className="flex items-center gap-2">
            ...
            {appState === 'config' && (
              <>
                ...
                <button onClick={() => setAppState('progress')} className="px-4 py-2 text-sm font-medium text-teal-400 bg-teal-400/10 rounded-lg hover:bg-teal-400/20 border border-teal-400/20">Progress</button>
              </>
            )}
                  ...
                  {pausedSessions.map(ps => (
                    <div key={ps.session_id} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5">
                      <div>
                        ...
                        <span className="text-xs text-slate-500 ml-3">{ps.domains.join(', ')}</span>
                      </div>
