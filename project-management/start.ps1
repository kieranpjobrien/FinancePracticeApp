...
$backendProc = Start-Process -FilePath $venvPython `
    -ArgumentList "-m", "uvicorn", "main:app", "--host", "127.0.0.1", "--port", "8009" `
...
for ($i = 0; $i -lt 15; $i++) {
    ...
    try {
        $null = Invoke-WebRequest -Uri "http://127.0.0.1:8009/" -UseBasicParsing -TimeoutSec 2
        $ready = $true
        break
    } catch {}
}
...
if ($frontendProc -and -not $frontendProc.HasExited) {
    ...
    # Also kill any child node processes from npm run dev
